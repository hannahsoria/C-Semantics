...
char *intToString(void *x) {
    char *out = (char*)malloc(10); // allocate memory
    sprintf(out, "%d", *((int *)x)); // print out string
    return out;
}

char *account_toString(void *v) {
    Account *account = (Account *)v; // define account
    char *out = (char *)malloc(100); // allocate memory
    sprintf(out, "%s: %d", account->name, account->balance); // print out string
    return out;
}
